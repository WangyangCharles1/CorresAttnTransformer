# -*- coding: utf-8 -*-
#""""""""""""""""""""""""""""""""""""" Powered By WHU MVP Lab """"""""""""""""""""""""""""""""""""""""""""""""
# @CLASS DESC:
#    - ResNet_Block: MLP-based deep architecture
#    - Covar_Attn: Second-order Covariance Normalized Channel Attention
#    - MultiHead_Attn: Spatial MultiHead Attention structure
#    - MultiHead_Covar_Attn: Second-order Covariance Normalized Channel Attention with Multi-head mechanism
#    - Architecture: The network architecture of Correspondence Attention Transformer
#
# @FUNC DESC:
#    - batch_symeig: eigendecomposition of batch data
#    - weighted_8points: weight 8-points algorithm for a regression of essential/fundamental matrix
#"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""

import torch
import torch.nn as nn
from torch.nn import functional as F
from torch.autograd import Variable
import numpy as np
import math
from loss import batch_episym
import MPNCOV

class ResNet_Block(nn.Module):
    def __init__(self, inchannel, outchannel, stride=1, pre=True):
        super(ResNet_Block, self).__init__()
        self.pre = pre
        self.right = nn.Sequential(
            nn.Conv2d(inchannel, outchannel, (1, 1), stride=(1, stride)#, padding=(0, 1)
            ),
            nn.BatchNorm2d(outchannel)
        )
        self.left = nn.Sequential(
            nn.Conv2d(inchannel, outchannel, (1, 1), stride=(1, stride)#, padding=(0, 1)
            ),
            nn.InstanceNorm2d(outchannel),
            nn.BatchNorm2d(outchannel),
            nn.ReLU(inplace=True),
            nn.Conv2d(outchannel, outchannel, (1, 1), stride=1#, padding=(0, 1)
            ),
            nn.InstanceNorm2d(outchannel),
            nn.BatchNorm2d(outchannel)
        )
    def forward(self, x):
        x1 = self.right(x) if self.pre is True else x
        out = self.left(x)       
        out = out + x1
        
        return F.relu(out)
        
class Covar_Attn(nn.Module):
    def __init__(self, channel):
        super(Covar_Attn, self).__init__()

    def forward(self, x):
        batch_size, C, h, w = x.shape  # x: NxCxHxW
        
        ## MPN-COV
        cov_mat = MPNCOV.CovpoolLayer(x) # Global Covariance pooling layer
        cov_mat_sqrt = MPNCOV.SqrtmLayer(cov_mat,5) # Matrix square root layer( including pre-norm,Newton-Schulz iter. and post-com. with 5 iteration)
        ##
        cov_mat_sum = torch.mean(cov_mat_sqrt,1)
        cov_mat_sum = cov_mat_sum.view(batch_size,C,1,1)
        y_cov = cov_mat_sum.repeat(1, 1, h, w)
        
        return y_cov*x
        
class MultiHead_Attn(nn.Module):
    def __init__(self, inchannel, outchannel, weight_out= False):
        super(MultiHead_Attn, self).__init__()
        self.group = 4
        self.group_norm = nn.GroupNorm(self.group, inchannel)
        self.linear = nn.Conv2d(inchannel, inchannel, kernel_size=(1,1), stride=(1,1), padding=0, bias=False)
        self.resnet = ResNet_Block(inchannel, outchannel, stride=1, pre=True)
    
    def forward(self, x):
    
        batch_size, c, num_pts = x.shape[0], x.shape[1], x.shape[2]
        
        feat = self.linear(x)                           
        feat_groups = feat.chunk(self.group, dim=1)     
        attn_groups = []
        for group in feat_groups:
            group_t = group.transpose(1,3)
            group_mul = torch.matmul(group_t, group_t.transpose(2,3))
            group_div = torch.div(group_mul ,np.sqrt(128))  
            s = F.softmax(group_div, dim=-1)  
            attn = torch.matmul(s, F.elu(group_t)).transpose(1,3)
            attn_groups.append(attn)
        
        feat_attn = torch.cat(attn_groups, dim=1)        
        feat_attn = self.group_norm(feat_attn + x)
        
        x_out = self.resnet(feat_attn)
        
        return x_out

class MultiHead_Covar_Attn(nn.Module):
    def __init__(self, inchannel, outchannel, weight_out= False):
        super(MultiHead_Covar_Attn, self).__init__()
        self.group = 4
        self.group_norm = nn.GroupNorm(self.group, inchannel)
        self.linear = nn.Conv2d(inchannel, inchannel, kernel_size=(1,1), stride=(1,1), padding=0, bias=False)
        self.resnet = ResNet_Block(inchannel, outchannel, stride=1, pre=True)
        self.ca = Covar_Attn(channel=inchannel)
    
    def forward(self, x):
    
        batch_size, c, num_pts = x.shape[0], x.shape[1], x.shape[2]
        feat = self.linear(x)                            
        feat_groups = feat.chunk(self.group, dim=1)      
        attn_groups = []
        for group in feat_groups:
            attn = self.ca(group)
            attn_groups.append(attn)
        
        feat_attn = torch.cat(attn_groups, dim=1)       
        feat_attn = self.group_norm(feat_attn + x)
        
        x_out = self.resnet(feat_attn)
        
        return x_out
        

class Architecture(nn.Module):
    def __init__(self, net_channels=128, input_channel=4, net_depth=6):
        super(Architecture, self).__init__()
        self.use_covarattn = True

        self.norm = nn.InstanceNorm2d(net_channels)  
        self.conv1 = nn.Sequential(
            nn.Conv2d(1, net_channels, (1, input_channel), stride=(1, input_channel),bias=True),
            nn.BatchNorm2d(net_channels),
            nn.ReLU(),
        )
                 
        self.conv2 = nn.Sequential(
            nn.Conv2d(net_channels, net_channels, (1, 1)),
            nn.InstanceNorm2d(net_channels),
            nn.BatchNorm2d(net_channels),
            nn.ReLU(),
        )
        self.final_conv = nn.Sequential(
            nn.Conv2d(net_channels, 1, (1, 1)),
        ) 
        if self.use_covarattn:
            self.layer = self.make_layers(ResNet_Block, MultiHead_Covar_Attn, net_channels, net_depth)
        else:
            self.layer = self.make_layers(ResNet_Block, MultiHead_Attention, net_channels, net_depth)
        
    def make_layers(self, ResNet_Block, Attn_Module, net_channels, net_depth):
        layers = []
        for _ in range(net_depth-1):
            layers.append(Attn_Module(net_channels, net_channels, weight_out= False))
        return nn.Sequential(*layers)
        
    
    def forward(self, x):
        out = self.conv1(x)
        batch_size, num_pts = x.shape[0], x.shape[2]
        out = self.layer(out)  
        out = self.final_conv(out)
        out = out.view(out.size(0), -1)
        out = out
        w = torch.tanh(out)
        w = F.relu(w)
        e_hat = weighted_8points(x[:,:,:,:4], w)
        x1, x2 = x[:,0,:,:2], x[:,0,:,2:4]
        
        return out, e_hat


def batch_symeig(X):
    # it is much faster to run symeig on CPU
    X = X.cpu()
    b, d, _ = X.size()
    bv = X.new(b,d,d)
    for batch_idx in range(X.shape[0]):
        e,v = torch.symeig(X[batch_idx,:,:].squeeze(), True)
        bv[batch_idx,:,:] = v
    bv = bv.cuda()
    return bv
    
    
    
def weighted_8points(x_in, weights):
    # x_in: batch * 1 * N * 4
    x_shp = x_in.shape
    # Turn into weights for each sample
    #weights = torch.relu(torch.tanh(logits))
    x_in = x_in.squeeze(1)
    
    # Make input data (num_img_pair x num_corr x 4)
    xx = torch.reshape(x_in, (x_shp[0], x_shp[2], 4)).permute(0, 2, 1)

    # Create the matrix to be used for the eight-point algorithm
    X = torch.stack([
        xx[:, 2] * xx[:, 0], xx[:, 2] * xx[:, 1], xx[:, 2],
        xx[:, 3] * xx[:, 0], xx[:, 3] * xx[:, 1], xx[:, 3],
        xx[:, 0], xx[:, 1], torch.ones_like(xx[:, 0])
    ], dim=1).permute(0, 2, 1)
    wX = torch.reshape(weights, (x_shp[0], x_shp[2], 1)) * X
    XwX = torch.matmul(X.permute(0, 2, 1), wX)
    

    # Recover essential matrix from self-adjoing eigen
    v = batch_symeig(XwX)
    e_hat = torch.reshape(v[:, :, 0], (x_shp[0], 9))

    # Make unit norm just in case
    e_hat = e_hat / torch.norm(e_hat, dim=1, keepdim=True)
    return e_hat