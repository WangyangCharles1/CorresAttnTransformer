...
class MatchLoss(object):
  ...
  def run(self, global_step, data, logits, e_hat):
    ...
    '''
    weight_losses = -torch.log(torch.sigmoid(c * mean) + np.finfo(float).eps.item())
    weight_loss_p = torch.sum(weight_losses * is_pos, dim=1)
    weight_loss_n = torch.sum(weight_losses * is_neg, dim=1)
    weight_loss = torch.mean(weight_loss_p * 0.5 / num_pos + weight_loss_n * 0.5 / num_neg)
    #print("weight_loss:", weight_loss.data)
    '''
    ...
    if self.loss_classif > 0:
        ...
        #loss += self.loss_weight * weight_loss
