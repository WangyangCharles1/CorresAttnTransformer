...
def test_process(mode, model, cur_global_step, data_loader, config):
    ...
    yhat_list = []
    ehat_list = []
    eval_step, eval_step_i, num_processor = 100, 0, 16
    time_list = []
    with torch.no_grad(): 
        for test_data in loader_iter:
            ...
            mask = (y_hat > 0).type(torch.IntTensor)
            ...
            #e_gt = np.matmul(np.matmul(np.linalg.inv(test_data['K2s']).T, e_gt), np.linalg.inv(test_data['K1s']))
            #e_gt = np.array(e_gt, dtype='float')
            ehat_list += [e_hat.detach().cpu().numpy()]
        ...
        if len(pool_arg) > 0:
            results += get_pool_result(num_processor, test_sample, pool_arg)
    avg_t = np.mean(np.asarray(time_list))
...
def test(data_loader, model, config):
    ...
    print("network have {} MB paramerters in total".format(num_params / 1e6))
