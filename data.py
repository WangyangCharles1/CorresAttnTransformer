...
class CorrespondencesDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.config.use_fundamental:
            cx1 = np.array(self.data['cx1s'][str(index)])
            cy1 = np.array(self.data['cy1s'][str(index)])
            cx2 = np.array(self.data['cx2s'][str(index)])
            cy2 = np.array(self.data['cy2s'][str(index)])
            f1 = np.array(self.data['f1s'][str(index)])
            f2 = np.array(self.data['f2s'][str(index)])
            K1 = np.array([
                [f1[0], 0, cx1[0]],
                [0, f1[0], cy1[0]],
                [0, 0, 1]
